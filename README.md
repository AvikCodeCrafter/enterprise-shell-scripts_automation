# Avik Kumar Banerjee - DevOps Bash Tools
...
- `github/*.sh` - [GitHub](https://github.com/) API / CLI scripts:
  ...
  - `github_forked_add_remote.sh` - quickly adds a forked repo as a remote from an interactive menu list of forked repos
  ...
  - `github_forks_containing_commit.sh` - finds all forks of a repo that contain a given commit hash to find private information for removal requests
...
- `spotify_artist_tracks.sh` - gets all track URIs for a given artist, from both albums and singles for chain loading to playlists
